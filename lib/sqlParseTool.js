...
function parseOneSource(part){
  ...
  if(idx === -1){
    ...
    if(part[1].text !== "("){throw new Error("Source处格式错误");}
    var innerLev = 0;
    var j;
    ...
    if(j === part.length){throw new Error("source处括号不匹配");}
    //列入db.table这样的源
  }else{
  ...
  if(part.length === 0){
    ...
    if(part.length >= 3 || (part.length === 2 && !(/^as$/i.test(part[0].text))) || (part[part.length-1].type !== Lexter.types.KEYWORD && part[part.length-1].type !== Lexter.types.VARIABLE)){throw new Error("Source处格式错误");}
    res = {
...
function parseOneWhere(part){
  ...
  //如果关系是类似 in , like 这样的关键字
    ...
    if(relate.text.toLowerCase() === "between"){
      ...
      res["values"] = [part[3],part[5]];
    }else if(relate.text.toLowerCase() === "in"){
      res["relate"] = RELATE[relate.text.toLowerCase()];
      res["values"] = [];
      for(var j = 3;j < part.length-1; j++){
        if(part[j].text === ","){continue;}
        res["values"].push(part[j]);
      }
